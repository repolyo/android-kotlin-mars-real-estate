...
class DetailViewModel(@Suppress("UNUSED_PARAMETER") marsProperty: MarsProperty, app: Application) : AndroidViewModel(app) {
    ...
    val displayPropertyPrice = Transformations.map(selectedProperty) {
        app.applicationContext.getString(
            when (it.type) {
                MarsApiFilter.SHOW_RENT.value -> R.string.display_price_monthly_rental
    ...
    val displayPropertyType = Transformations.map(selectedProperty) {
        app.applicationContext.getString(R.string.display_type,
                app.applicationContext.getString(
                        when (it.type) {
                            MarsApiFilter.SHOW_RENT.value -> R.string.type_rent
                            MarsApiFilter.SHOW_BUY.value -> R.string.type_buy
                            else ->  R.string.show_all
                        })
        )
    }
}
